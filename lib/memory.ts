...
export function initialState():State{return {schema:3,revision:1,extracted:false,sourceText:originalSource,sourceAuthor:'Choi（原始发言）',sourceHistory:[],sourceDirty:false,audit:[],sessions:[],memories:[
 ...
 {id:'data',title:'B 组接触电阻',text:'B 组中位数 690 Ω·μm，n=5；A 组 820 Ω·μm，n=6。替代旧版 610。',value:690,status:'pending',owner:'Choi',scope:'team',project:'M09',source:'09.23 · Choi · 18:42',meeting:2,speaker:3,version:2},
 {id:'decision',title:'阶段良率门槛',text:'两组初筛通过率均至少 75%，从 9 月 23 日开始适用。',status:'pending',owner:'Park',scope:'team',project:'M09',source:'09.23 · Park · 02:10',meeting:2,speaker:0,version:1},
 {id:'screening',title:'器件初筛结果',text:'Lee 报告 A 组初筛通过 10/12，B 组 8/12。此处不是接触电阻提取样本数。',status:'confirmed',owner:'Lee',scope:'team',project:'M09',source:'09.23 · Lee · 12:06（预置核查为会议陈述）',meeting:2,speaker:2,version:1},
 {id:'hypothesis',title:'界面机制假设',text:'接触处理可能改变界面状态，尚未证明因果关系。',status:'confirmed',owner:'Wang',scope:'team',project:'M09',source:'09.02 · Wang · 24:15（预置保留为假设）',meeting:0,speaker:4,version:1},
 {id:'tasks',title:'待完成的研究工作',text:'Han 修订月报并补齐器件排除原因；Choi 补充测试条件；Lee 确认清洗对照安排。没有证据表明任务已完成。',status:'confirmed',owner:'Han',scope:'team',project:'M09',source:'09.23 · Han · 29:30（预置行动项）',meeting:2,speaker:5,version:1},
 {id:'private',title:'Wang 的个人研究笔记',text:'Wang 的假设：清洗步骤可能影响结果，仍需实验验证。',status:'confirmed',owner:'Wang',scope:'private',project:'M09',source:'Wang · 个人笔记（虚构示例）',meeting:-1,speaker:4,version:1},
 {id:'other',title:'另一个项目的初步记录',text:'P08 项目曾观察到约 30% 的变化，材料与统计口径不同。',status:'confirmed',owner:'Kim',scope:'team',project:'P08',source:'P08 · 另一项目（虚构示例）',meeting:-1,speaker:1,version:1}
]}}
function change(s:State,description:string):State{return {...s,revision:s.revision+1,audit:[description,...s.audit].slice(0,50)}}
export function act(s:State,id:MemoryId,action:'confirm'|'reject'|'forget'|'reconsider'|'share'|'unshare',actor:string):State{
 const m=s.memories.find(x=>x.id===id);if(!m)throw new Error('记忆不存在');
 if((action==='share'||action==='unshare')&&(m.id!=='private'||actor!==m.owner))throw new Error('仅笔记作者能更改共享范围');
 ...
 if(action==='reconsider'&&m.status!=='rejected')throw new Error('仅被拒绝的候选可重新核查');
 if(m.status==='forgotten')throw new Error('已忘记的记忆不能直接恢复，请从来源重新提取');
 const next={...m,version:m.version+1};
 if(action==='confirm')next.status='confirmed';if(action==='reject')next.status='rejected';
 if(action==='forget'){next.status='forgotten';next.text='';delete next.value}
 if(action==='reconsider')next.status='pending';if(action==='share')next.scope='team';if(action==='unshare')next.scope='private';
 return change({...s,memories:s.memories.map(x=>x.id===id?next:x)},`${actor} · ${action==='confirm'?'确认':action==='reject'?'不保存':action==='forget'?'忘记':action==='reconsider'?'重新核查':action==='share'?'共享为个人假设':'撤回共享'} · ${m.title}`)
}
...
export function recall(s:State,actor:string,question:string,ignore:MemoryId[]=[]):Answer{
 ...
 for(const m of s.memories){
  ...
  const item={id:m.id,title:m.title,source:m.source,meeting:m.meeting,speaker:m.speaker,reason:''};
  let reason='';if(m.project!=='M09')reason='项目不同，材料与口径不一致';else if(m.status==='forgotten')reason='已忘记，不检索正文';else if(m.status==='rejected')reason='用户选择不保存';else if(m.status!=='confirmed')reason='尚未确认或来源有变更';else if(ignore.includes(m.id))reason='你选择本次不用';else if(!relevant)reason='与本次任务不相关';
  if(reason)skipped.push({...item,reason});else used.push({...item,reason:m.id==='private'?'作者的个人假设，不能当团队结论':`同项目、当前有效；${m.id==='hypothesis'?'仅以假设语气使用':'支持本次研究任务'}`});
 }
 const has=(id:MemoryId)=>used.some(x=>x.id===id);const value=s.memories.find(m=>m.id==='data')?.value;
 const lines:string[]=[];
 if(!relevant)lines.push('当前演示只支持研究汇报、项目进展和会议准备。这个问题与 M09 记忆无关，因此没有调用项目记录。');
 else {
  if(has('goal'))lines.push('本月目标是接触电阻相对对照组降低至少 20%，目标不等于实际结果。');
  if(has('data')&&value){const percent=(820-value)/820*100;lines.push(`经负责人确认，A 组中位接触电阻为 820 Ω·μm（n=6），B 组为 ${value} Ω·μm（n=5）。相对${percent>=0?'降低':'升高'} ${Math.abs(percent).toFixed(1)}%。${has('goal')?(percent>=20?'数值上达到 20% 目标，但仍须检验实验可靠性。':'未达到 20% 目标。'):'本次未调用目标，不判断目标是否达成。'}`)}else lines.push('本次没有可用的已确认电阻数据，不引用旧数字，也不判断电阻目标是否达成。');
  if(has('decision'))lines.push('9 月 23 日起，阶段门槛为两组初筛通过率均至少 75%。');
  if(has('screening'))lines.push(`据 Lee 的会议陈述，A 组初筛通过 10/12（83.3%），B 组 8/12（66.7%）。${has('decision')?'B 组未达到阶段门槛。':'本次未调用有效门槛，不判断是否达标。'}初筛数量不等于接触电阻提取样本数。`);
  if(has('hypothesis'))lines.push('界面状态变化仍是待验证假设，不能写成已证明的因果关系。');
  if(has('tasks'))lines.push('下一步：Han 修订月报和器件排除原因；Choi 补齐测试条件；Lee 确认清洗对照安排。任务尚无完成证据。');
  if(has('private'))lines.push('补充观点：Wang 的个人假设是清洗步骤可能影响结果。即使共享，也不代表团队共识。');
  lines.push('上述内容来自会议或笔记记录；未接入原始实验文件，不能据此推断统计显著性。');
 }
