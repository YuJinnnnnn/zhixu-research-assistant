...
test('transcript edits retain every version, author and timestamp and survive reload', () => {
  let s = initialState();
  const original = transcript(s, 0, 0);
  s = saveTranscript(
    s,
    0,
    0,
  ...
  assert.equal(versions(s, 0, 0).length, 3);
  assert.equal(versions(s, 0, 0)[0].text, original);
  assert.equal(versions(s, 0, 0)[1].author, 'Kim');
  assert.equal(versions(s, 0, 0)[2].at, '2026-09-16T02:00:00Z');
...
test('Choi transcript correction retains history and triggers existing conservative extraction', () => {
  ...
  assert.equal(versions(s, 2, 3).length, 2);
  assert.ok(versions(s, 2, 3)[1].at);
  assert.equal(s.memories.find((m) => m.id === 'data').status, 'pending');
});

test('an empty stored history still displays the original version', () => {
  const s = { ...initialState(), transcriptVersions: { '0:0': [] } };
  const history = versions(s, 0, 0);
  assert.equal(history.length, 1);
  assert.equal(history[0].version, 1);
  assert.equal(history[0].author, 'Park');
});
